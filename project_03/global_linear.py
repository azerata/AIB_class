...
def c(i:int, j:int, matrix, gap_cost:int, seq1:str, seq2:str, cost)->int:
    ''' 
    Input: 
    ...
    if matrix[i,j] is None:
        ...
        if i > 0 and j > 0:
            v1 = c(i-1,j-1, matrix, gap_cost, seq1, seq2, cost) + cost(seq1[i-1],seq2[j-1])
        if i > 0 and j >= 0:
            v2 = c(i-1,j, matrix, gap_cost, seq1, seq2, cost) + gap_cost
        if i >= 0 and j > 0:
            v3 = c(i, j-1, matrix, gap_cost, seq1, seq2, cost) + gap_cost
        if i == 0 and j == 0:
            v4 = 0
        matrix[i,j] = min(v1, v2, v3, v4)
    return matrix[i,j]

def build_alignment(seq1:str, seq2:str, gap_score:int, cost):
    '''
    ...
    out = c(i, j, matrix, gap_score, seq1, seq2, cost)
    return  (matrix, out)

def backtrack(matrix, i,j, seq1, seq2, gap_cost, cost, out1 = [], out2 = []):
    T = matrix
    if (i > 0) and (j > 0) and (T[i,j] == (T[i-1,j-1] + cost(seq1[i-1],seq2[j-1]))):
        out1.append(seq1[i-1])
        out2.append(seq2[j-1])
        backtrack(T, i-1, j-1, seq1, seq2, gap_cost, cost, out1, out2)
    elif (i > 0) and (j >= 0) and (T[i,j] == (T[i-1,j] + gap_cost)):
        out1.append(seq1[i-1])
        out2.append("-")
        backtrack(T, i-1, j, seq1, seq2, gap_cost, cost, out1, out2)
    elif (i >= 0) and (j > 0) and (T[i,j] == (T[i,j-1] + gap_cost)):
        out1.append("-")
        out2.append(seq2[j-1])
        backtrack(T, i, j-1, seq1, seq2, gap_cost, cost, out1, out2)
    return("".join(out1)[::-1], "".join(out2)[::-1] )

def optimal_alignment(seq1:str, seq2:str, gap_cost:int, cost):
    pip = build_alignment(seq1, seq2, gap_cost, cost)
    out = backtrack(pip[0], len(seq1), len(seq2), seq1, seq2, gap_cost, cost)
    return out
...
def optimal_score_matrix(sequences:list, gap_cost:int, cost):
    k = len(sequences)
    M = np.zeros((k,k))
    for i in range(k):
        for j in range(k):
            S = build_alignment(sequences[i],sequences[j],gap_cost, cost)[1]
            M[i,j] = S
    return M
